...
public class ExcelDriver {
	static Logger log = Logger.getLogger(ExcelDriver.class.getName());
	...
	public static Map getMultipleData(String dataFileName, String sheetName, int columnNumber) throws IOException {
		...
		for (int k = 2; k < (dataSize + 2); k++) {
			//"try .. catch" is used to prevent situation when getPhysicalNumberOfRows doesn't work correctly (blank rows are not recognized)
        	try {
	...
	public static Map getDataByColumn(String dataFileName, String sheetName, int columnNumber) throws IOException {
		Map<String, String> testData = new HashMap<String, String>();
		// Create stream for reading from file
		InputStream input = new FileInputStream(dataFileName);
		// Get Excel WorkBook from input stream
